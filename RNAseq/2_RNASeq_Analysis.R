...
output_dir		  	= 'output/'
bam_dir			    = 'bam_dir/'
genes.gtf.file		= '/data/genomes/rn6/rn6.refGene.gtf'
...
#####################  Packages  ###############################################
################################################################################
## You may use library() instead of require()
## require() is better suited to be used in a script or inside a function
require(Rsubread) || stop("The Rsubread library is not available!")
...
require(dplyr) || stop("The dplyr library is not available!")
require(ggpubr) || stop("The ggpubr library is not available!")
require(org.Hs.eg.db) || stop("The org.Hs.eg.db library is not available!")
require(org.Rn.eg.db) || stop("The org.Rn.eg.db library is not available!")
require(pca3d) || stop("The pca3d library is not available!")
require(EnsDb.Hsapiens.v86) || stop("The EnsDb.Hsapiens.v86 library is not available!")
require(richR) || stop("The richR library is not available!")
require(GO.db) || stop("The GO.db library is not available!")
require(dendextend) || stop("The dendextend library is not available!")
require(VennDetail) || stop("The VennDetail library is not available!")


################################################################################
## Install any missing packages
################################################################################
## Packages from CRAN: use 'install.packages()'
## > install.packages(c("gplots","reshape2","ggplot2","NMF","dplyr","RColorBrewer",
##      "gplots","pheatmap","ggpubr","GenomicFeatures","dendextend","pca3d"))
## Packages from Bioconductor, use 'BiocManager::install()'
## > BiocManager::install(c("DESeq2", "Rsubread","GenomicFeatures","RDAVIDWebService","biomaRt",
##        "org.Mm.eg.db","org.Hs.eg.db","EnsDb.Hsapiens.v86","GO.db","org.Rn.eg.db"))

# ## You need to have BiocManager package available before installing Bioconductor package
...
################################################################################
group<-read.delim("samples.txt",sep="\t",header=T)
group.bak<-group
...
count_data <- data.frame(cbind(count_data, 
    ...
    FirstLimbicMean=apply(count_data[,(condition == "1st limbic")],1,mean),
    ThreeClassV2Mean=apply(count_data[,(condition == "3 Class V")],1,mean),
    FiveADsMean=apply(count_data[,(condition == "5ADs")],1,mean)))
fpkm_data <- data.frame(cbind(fpkm_data, 
    ...
    FirstLimbicMean=apply(fpkm_data[,(condition == "1st limbic")],1,mean),
    ThreeClassV2Mean=apply(fpkm_data[,(condition == "3 Class V")],1,mean),
    FiveADsMean=apply(fpkm_data[,(condition == "5ADs")],1,mean)))
...
# Bb_1h_group = sort(colnames(count_data)[grep("Bb_1hIFNA", colnames(count_data))])
# Bb_24h_group = sort(colnames(count_data)[grep("Bb_24hIFNA", colnames(count_data))])
# Bb_Contrl_group = sort(colnames(count_data)[grep("Bb_Control",colnames(count_data))])
# UT_1h_group = sort(colnames(count_data)[grep("UT_1hIFNA", colnames(count_data))])
# UT_24h_group = sort(colnames(count_data)[grep("UT_24hIFNA", colnames(count_data))])
# UT_Contrl_group = sort(colnames(count_data)[grep("UT_Control",colnames(count_data))])
# count_data[rownames(count_data)[grep("^ENSG00000169245",rownames(count_data))],c(Bb_1h_group,Bb_Contrl_group)]
# fpkm_data[rownames(fpkm_data)[grep("^ENSG00000169245",rownames(fpkm_data))],c(Bb_1h_group,Bb_Contrl_group)]
...
rld_mat_ext <- data.frame(cbind(rld_mat, 
    ...
    FirstLimbicMean=apply(rld_mat[,(condition == "1stlimbic")],1,mean),
    ThreeClassV2Mean=apply(rld_mat[,(condition == "3ClassV2")],1,mean),
    FiveADsMean=apply(rld_mat[,(condition == "5ADs")],1,mean)))
...
DEGSummary<-as.data.frame(t(matrix(unlist(lapply(DEGfiles, function(x){
  shortname<-sub(paste0("_DEtable_padj0.05_",analysis_date,".txt"),'',x)
  df<-read.delim(x)
  return(c(shortname, dim(df)[1]))})),nrow=2)))
colnames(DEGSummary)<-c("Comparison","#ofDEGs")

## save the DEG list summary
sink(paste0("Summary_DE_count_",analysis_date,".txt"))
DEGSummary
sink()
...
## test example
DEGMaster[["SalineSham^vs^Sham"]]

## Some select overlap analysis
## '3ClassV1_vs_Sham' with 963 DEGs
## '5ADs_vs_Sham' with 9 DEGs
## '3ClassV2_vs_SalineSham' with 330 DEGs
## '1stlimbic_vs_SalineSham' with 1175 DEGs

DE.output.KIKA<-list(FiveADs_vs_Sham = rownames(DEGMaster[["5ADs_vs_Sham"]]), 
                         Firstlimbic_vs_SalineSham = rownames(DEGMaster[["1stlimbic_vs_SalineSham"]]),
                         ThreeClassV1_vs_Sham = rownames(DEGMaster[["3ClassV1_vs_Sham"]]),
                         ThreeClassV2_vs_SalineSham = rownames(DEGMaster[["3ClassV2_vs_SalineSham"]]))
DE.output.KI<-list(FiveADs_vs_Sham = rownames(DEGMaster[["5ADs_vs_Sham"]]),
                   ThreeClassV1_vs_Sham = rownames(DEGMaster[["3ClassV1_vs_Sham"]]))
DE.output.KA<-list(Firstlimbic_vs_SalineSham = rownames(DEGMaster[["1stlimbic_vs_SalineSham"]]),
                   ThreeClassV2_vs_SalineSham = rownames(DEGMaster[["3ClassV2_vs_SalineSham"]]))
DE.output.early<-list(FiveADs_vs_Sham = rownames(DEGMaster[["5ADs_vs_Sham"]]), 
                         Firstlimbic_vs_SalineSham = rownames(DEGMaster[["1stlimbic_vs_SalineSham"]]))
DE.output.advanced<-list(ThreeClassV1_vs_Sham = rownames(DEGMaster[["3ClassV1_vs_Sham"]]),
                         ThreeClassV2_vs_SalineSham = rownames(DEGMaster[["3ClassV2_vs_SalineSham"]]))

## venn diagram overlap analysis
venn <- VennDetail::venndetail(DEoutputs)
pdf("VennDiagram_four-DEGsets.pdf")
plot(venn)
dev.off()
...
# 
# 
# write.csv (cbind(result(venn.Bh)[,1], result(venn.Bh, wide=TRUE)), 
#            paste0(output_dir,"/","VennDetail_Bh-2way.txt"), quote=F, row.names=F)
# write.csv (cbind(result(venn.UT)[,1], result(venn.UT, wide=TRUE)), 
#            paste0(output_dir,"/","VennDetail_UT-2way.txt"), quote=F, row.names=F)
# write.csv (cbind(result(venn.BhUT)[,1], result(venn.BhUT, wide=TRUE)), 
#            paste0(output_dir,"/","VennDetail_BhUT-4way.txt"), quote=F, row.names=F)
